...
# descriptions
aws_ami_description="EBS AMI, copied from Instance Backed AMI"
date_fmt=$(date '+%F-%H-%M')
...
release=${string##*=}
echo $release
aws_ami_name="$id-$release-ebs-$date_fmt"
...
sudo -E $EC2_AMITOOL_HOME/bin/ec2-create-volume --size 10 --region $aws_region --availability-zone $aws_avail_zone
